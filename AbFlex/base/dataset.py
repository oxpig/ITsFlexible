...
class LoopGraphDataSet(Dataset):
    ...
    def __init__(self,
        interaction_dist: float=10, # cutoff interaction distance for generating graphs
        graph_mode: str='loop_context', # graph generation mode
        typing_mode='res_type', # mode for atom typing
        edge_encoding=['covalent', 'rbf'], # edge encoding
        aa_map_mode='extended', # mode for mapping amino acids to integers
    ...
    def _get_loop_context_graph(self, prot_df: pd.DataFrame, loop_chain: str, loop_resi: list):
        """Return graph composed of nodes in loops and surrounding context nodes
        ...
        loop_nodes = prot_df.loc[(prot_df["chain_id"] == loop_chain) & (prot_df["residue_number"].isin(np.arange(loop_resi[0], loop_resi[1]+1)))]
        loop_coords = loop_nodes.loc[:, ['x_coord', 'y_coord', 'z_coord']].to_numpy(dtype=np.float64)
        context_nodes = prot_df.loc[~((prot_df["chain_id"] == loop_chain) & (prot_df["residue_number"].isin(np.arange(loop_resi[0], loop_resi[1]+1)))), :]
        context_coords = context_nodes.loc[:, ['x_coord', 'y_coord', 'z_coord']].to_numpy(dtype=np.float64)
        ...
        _, neighbours_context = np.where(dist_loop_context < self.interaction_dist)
        ...
        # features: node xyz coordinates [cols 0-2], one-hot encoded res types [cols 3-13], label of whether loop/context [col 14]
        feats_loop = np.concatenate([loop_coords, loop_node_features, loop_node_bool], axis=-1)
        feats_context = np.concatenate([context_coords, context_node_features, context_node_bool], axis=-1)

        feats = np.concatenate([feats_loop, feats_context], axis=-2)

        #### EDGES ####
        edge_indices, edge_attr_full = self._edge_encoding(loop_nodes, loop_coords, 
                                                           context_nodes=context_nodes, context_coords=context_coords,
                                                           encodings=self.edge_encoding)

        return feats, edge_indices, edge_attr_full


    def _get_loop_graph(self, prot_df: pd.DataFrame, loop_chain: str, loop_resi: list):
        ...
        # features: node xyz coordinates [cols 0-2], one-hot encoded res types [cols 3-13]
        feats = np.concatenate([loop_coords, loop_node_features], axis=-1)
        
        #### EDGES ####
        edge_indices, edge_attr_full = self._edge_encoding(loop_nodes, loop_coords, encodings=self.edge_encoding)
    ...
    def _get_node_features(self, df: pd.DataFrame):
        ...
        if mode == 'lmg':
            ...
            types = types.apply(lambda x: self.aa_map[x] if x in self.aa_map.keys() else 20).astype(np.int64)
            types = np.array(types)
            types = utils.get_one_hot(types, nb_classes=max(self.aa_map.values()) + 1)
            return types
        else:
            raise NotImplementedError(mode)

    
    def _edge_attr_intra_inter(self, intra_dst_loop, intra_dst_context, inter_dst_context):
    ...
    def _edge_attr_intraLOOP_intraCONTEXT_inter(self, intra_dst_loop, intra_dst_context, inter_dst_context):
        '''3 class one-hot encoding for intra_loop, intra_context, inter_context_loop '''
        edge_attr = np.zeros((len(intra_dst_loop)+len(intra_dst_context)+len(inter_dst_context)+len(inter_dst_context), 3))
    ...
    def _edge_attr_covalent(self, loop_nodes, context_nodes, intra_src_loop, intra_dst_loop,
                            intra_src_context, intra_dst_context, inter_src_loop, inter_dst_context):
        '''one-hot encoding for covalent bonds'''
        # 1 intra_loop
        interval = np.abs(loop_nodes.iloc[intra_src_loop].residue_number.values - loop_nodes.iloc[intra_dst_loop].residue_number.values)
        ...
        if self.graph_mode == 'loop_context':
            # 2 intra_context
            interval = np.abs(context_nodes.iloc[intra_src_context].residue_number.values - context_nodes.iloc[intra_dst_context].residue_number.values)
            idx = np.argwhere(interval == 1)
            intra_context_covalent = np.zeros_like(intra_src_context)
            intra_context_covalent[idx] = 1

            # 3 inter
            interval = np.abs(loop_nodes.iloc[inter_src_loop].residue_number.values - context_nodes.iloc[inter_dst_context].residue_number.values)
            idx = np.argwhere(interval == 1)
    ...
    def _rbf(self, distances, count=8):
        '''Encodes a distance array in a radial basis function representation with n=count basis functions'''
        min, max = 0., 10
    ...
    def _edge_encoding(self, loop_nodes, loops_coords, context_nodes=None, context_coords=None,
                       encodings=None):
        ...
        intra_src_loop, intra_dst_loop = np.where(dist_intra_loop < self.interaction_dist)
        
        if self.graph_mode == 'loop_context':
            # 2 intra_context: interactions between context nodes
            dist_intra_context = cdist(context_coords, context_coords)
            intra_src_context, intra_dst_context = np.where(dist_intra_context < self.interaction_dist)

            # 3 inter: interactions between loop & context
            dist_inter = cdist(loops_coords, context_coords)
            inter_src_loop, inter_dst_context = np.where(dist_inter < self.interaction_dist)
            ...
            edge_src = np.concatenate([
                ...
                [i + context_offset for i in inter_dst_context] # make inter edges bidriectional
            ])
            ...
            dist = np.concatenate([dist_intra_loop[intra_src_loop, intra_dst_loop],
        ...
        if 'intra/inter' in encodings:
            edge_attrs.append(self._edge_attr_intra_inter(intra_dst_loop, intra_dst_context, inter_dst_context))
        if 'intraCONTEXT/intraLOOP/inter' in encodings:
            edge_attrs.append(self._edge_attr_intraLOOP_intraCONTEXT_inter(intra_dst_loop, intra_dst_context, inter_dst_context))
        if 'covalent' in encodings:
            edge_attrs.append(self._edge_attr_covalent(loop_nodes, context_nodes, intra_src_loop, intra_dst_loop,
                                                       intra_src_context, intra_dst_context, inter_src_loop, inter_dst_context))
        if 'rbf' in encodings:
    ...
    def _generate_graph_dict(self, prot_df: pd.DataFrame, loop_chain: str, loop_resi: list):
    ...
    def _parse_graph(self, graph_dict: dict, label:np.ndarray, protloop_def:dict, predict=False):
    ...
    def populate(self, input_file: Path, overwrite: bool=False):
        """Extract information from input files and save in protloop_defs list of lists

        Args:
            input_file: cvs containing columns: labels, pdb [path to file], prot_chains, chain of selected loop, resi of selected loop
        """
    ...
    def __getitem__(self, idx: int, force_recalc: bool = False):
        ...
        # if not create and save typed files
            ...
            pdb_df = utils.parse_pdb_to_parquet(protloop_def['pdb'], typed_pdb, lmg_typed=False, ca=True)
        ...
        # generate graph
        graph_dict = self._generate_graph_dict(prot_df, loop_chain=protloop_def['chain'], loop_resi=[protloop_def['resi_start'], protloop_def['resi_end']])
        graph = self._parse_graph(graph_dict, label, protloop_def=protloop_def, predict=self.predict)
